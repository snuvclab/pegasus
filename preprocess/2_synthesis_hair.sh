...
do
  ...
  echo $video
  IFS='.' read -r -a array <<< $video
  python object_masking_by_bisenet.py --base_path $video_folder/$subject_name/"${array[0]}" --input_dir image_rendering --output_dir mask_object --label $object_name --kernel_size 0
done
